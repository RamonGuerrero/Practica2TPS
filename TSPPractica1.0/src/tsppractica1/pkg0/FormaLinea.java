...
public class FormaLinea {
     ...
     */
    public boolean validarCodop(String busquedad){
        if(codop==null){
            llevaOperando=false;
            modoDir=null;
            return false;
            ...
            int indiceDir=busquedad.indexOf("|");
            int indiceCal=busquedad.indexOf("/");
            int indicePor=busquedad.indexOf("\\");
            if (busquedad.contains("SN")) {
                System.out.println(busquedad);
                modoDir=busquedad.substring(indiceDir, indiceCal);
                String auxC=busquedad.substring(indiceCal+2, indicePor);
                calBytes=Byte.parseByte(auxC);
                porBytes=0;
                totalBytes=calBytes;
                llevaOperando=false;
                return true;
                ...
                System.out.println(busquedad);
                modoDir=busquedad.substring(indiceDir, indiceCal);
                //String auxC=busquedad.substring(indiceCal+1, indicePor-1);
                //String auxP=busquedad.substring(indicePor+2);
                //calBytes=Byte.parseByte(auxC);
                //porBytes=Byte.parseByte(auxP);
                //totalBytes=calBytes+porBytes;
                llevaOperando=true;
                return true;
            }
            else{
    ...
    public String Automata() {
        ...
        if (null != aux) {
            switch (aux) {
                ...
                case "qf":
                    imprimirValido += modoDir + saltoLinea;
                    return imprimirValido;
                default:
